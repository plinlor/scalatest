...
/**
 * Implementation trait for class `FunSuite`, which represents
 * a suite of tests in which each test is represented as a function value.
 * 
 * <a href="FunSuite.html">`FunSuite`</a> is a class, not a trait,
 * to minimize compile time given there is a slight compiler overhead to
 * mixing in traits compared to extending classes. If you need to mix the
 * behavior of `FunSuite` into some other class, you can use this
 * trait instead, because class `FunSuite` does nothing more than
