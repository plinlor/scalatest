 ...
 * using a style consistent with traditional `WordSpec` tests.
 *
 * <table><tr><td class="usage">
 * '''Recommended Usage''':
 * `AsyncFunSpec` is intended to enable users of <a href="FunSpec.html">`FunSpec`</a>
 ...
 * }}}
 *
 * &ldquo;`it`&rdquo; is a method, defined in `AsyncWordSpec`, which will be invoked
 * by the primary constructor of `AddSpec`. You specify the name of the test as
 * a string between the parentheses, and the test code itself between curly braces.
 * The test code is a function passed as a by-name parameter to `it`, which registers
 * it for later execution. The result type of the by-name in an `AsyncWordSpec` must
 * be `Future[Assertion]`.
 * 
 *
 * In an `AsyncWordSpec` you write a one (or more) sentence specification for each bit of behavior you wish to
 ...
 * In short, you structure an `AsyncWordSpec` exactly like a `WordSpec`, but with
 * tests having result type `Assertion` or `Future[Assertion]`. For more examples
 * of structure, see the documentation for <a href="WordSpec.html">`WordSpec`</a>.
 * 
 *
 ...
 * }}}
 *
 * An `AsyncWordSpec`'s lifecycle has two phases: the ''registration'' phase and the
 ...
 * 
 *
 * Tests can only be registered with the `it` method while the `AsyncWordSpec` is
 * in its registration phase. Any attempt to register a test after the `AsyncWordSpec` has
 * entered its ready phase, ''i.e.'', after `run` has been invoked on the `AsyncWordSpec`,
 * will be met with a thrown `TestRegistrationClosedException`. The recommended style
 * of using `AsyncWordSpec` is to register tests during object construction as is done in all
 ...
 *
 * `AsyncWordSpec` extends <a href="AsyncTestSuite.html">`AsyncTestSuite`</a>, which provides an
 * implicit `scala.concurrent.ExecutionContext`
 * named `executionContext`. This
 * execution context is used by `AsyncWordSpec` to 
 ...
 * traditional <a href="WordSpec.html">`WordSpec`</a> with
 ...
 *
 * By default (unless you mix in `ParallelTestExecution`), tests in an `AsyncWordSpec` will be executed one after
 ...
 * 
 *
 * If you want the tests of an `AsyncWordSpec` to be executed in parallel, you
 ...
 * run in parallel very much like traditional (such as <a href="WordSpec.html">`WordSpec`</a>) tests run in
 ...
 * good intention of resurrecting the test at a later time, `AsyncWordSpec` adds a method
 ...
 *
 * One of the parameters to `AsyncWordSpec`'s `run` method is a <a href="Reporter.html">`Reporter`</a>, which
 * will collect and report information about the running suite of tests.
 * Information about suites and tests that were run, whether tests succeeded or failed, 
 * and tests that were ignored will be passed to the `Reporter` as the suite runs.
 * Most often the reporting done by default by `AsyncWordSpec`'s methods will be sufficient, but
 ...
 * the <a href="GivenWhenThen.html">`GivenWhenThen`</a> trait provides methods that use the implicit `info` provided by `AsyncWordSpec`
 ...
 *
 * `AsyncWordSpec` also provides a `markup` method that returns a <a href="Documenter.html">`Documenter`</a>, which allows you to send
 ...
 *
 * An `AsyncFunSpec`'s tests may be classified into groups by ''tagging'' them with string names.
 * As with any suite, when executing an `AsyncFunSpec`, groups of tests can
 * optionally be included and/or excluded. To tag an `AsyncFunSpec`'s tests,
 ...
 * create a matching tag for `AsyncFunSpec`s like this:
 * 
 *
 * {{{  <!-- class="stHighlight" -->
 ...
 * create a `Tag` object. A tag annotation (on the JVM, not Scala.js) allows you to tag all the tests of an `AsyncFunSpec` in
 ...
 * method defined in trait <a href="AsyncTestSuite.html">`AsyncTestSuite`</a>, a supertrait of `AsyncFunSpec`.
 * 
 *
 * Trait `AsyncFunSpec`'s `runTest` method passes a no-arg async test function to
 ...
 * the <a href="fixture/AsyncFunSpec.html">documentation for `fixture.AsyncFunSpec`</a>.
 * 
 *
 ...
 * To accomplish this in an `AsyncFunSpec`, you first place shared tests in
 * ''behavior functions''. These behavior functions will be
 * invoked during the construction phase of any `AsyncFunSpec` that uses them, so that the tests they contain will
 ...
 * stack fixture to use when running the tests. So in your `AsyncFunSpec` for `StackActor`, you'd invoke the
 ...
 * 
 *
 * You can define a behavior function that encapsulates these shared tests inside the `AsyncWordSpec` that uses them. If they are shared
 * between different `AsyncFunSpec`s, however, you could also define them in a separate trait that is mixed into
 * each `AsyncFunSpec` that uses them.
 ...
 * }}}
 *
 * Given these behavior functions, you could invoke them directly, but `AsyncWordSpec` offers a DSL for the purpose,
 ...
 * Although in an `AsyncWordSpec`, the `when`, `should`, `can` and `must` clause is a nesting construct analogous to
 ...
 * `AsyncFunSpec`, you'll need to pass in a prefix or suffix string to add to each test name. You can call
 ...
 *
 * <ul>
 * <li>`A Stack when non-empty should return non-empty StackInfo when Size is fired at non-empty stack actor: almost empty stack actor`</li>
 * <li>`A Stack when non-empty should return before and after StackInfo that has existing size and lastItemAdded as top when Peek is fired at non-empty stack actor: almost empty stack actor`</li>
 * <li>`A Stack when non-empty should return before and after StackInfo that has existing size - 1 and lastItemAdded as top when Pop is fired at non-empty stack actor: almost empty stack actor`</li>
 * </ul>
 *
 ...
 *
 * <ul>
 * <li>`A Stack when non-empty should return non-empty StackInfo when Size is fired at non-empty stack actor: almost full stack actor`</li>
 * <li>`A Stack when non-empty should return before and after StackInfo that has existing size and lastItemAdded as top when Peek is fired at non-empty stack actor: almost full stack actor`</li>
 * <li>`A Stack when non-empty should return before and after StackInfo that has existing size - 1 and lastItemAdded as top when Pop is fired at non-empty stack actor: almost full stack actor`</li>
 * </ul>
 */
