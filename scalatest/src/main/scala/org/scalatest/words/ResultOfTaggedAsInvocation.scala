 ...
 * }}}
 *
 * Rather than provide `in` and `ignore` methods directly, these
 * methods are provided after `taggedAs()` by implicit conversions
 * because the type passed to `in` (and `ignore`)
 * differs in a `FlatSpec` and a `fixture.FlatSpec`.
