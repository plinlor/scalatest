 ...
 * '''Recommended Usage''':
 * For teams coming from specs or specs2, `WordSpec` will feel familiar, and is often the most natural way to port specsN tests to 
 * ScalaTest. `WordSpec` is very prescriptive in how text must be written, so a good fit for teams who want a high degree of discipline 
 * enforced upon their specification text.
 * </td></tr></table>
 * 
 * Class `WordSpec` is so named because your specification text is structured by placing words after strings.
 ...
 * 
 *
 * ''Note: Class `WordSpec` is in part inspired by class `org.specs.Specification`, designed by
 * Eric Torreborre for the <a href="http://code.google.com/p/specs/" target="_blank">specs framework</a>.''
 * 
 *
 ...
 *
 * One of the parameters to `WordSpec`'s `run` method is a <a href="Reporter.html">`Reporter`</a>, which
 ...
 * the <a href="GivenWhenThen.html">`GivenWhenThen`</a> trait provides methods that use the implicit `info` provided by `WordSpec`
 ...
 *
 * `WordSpec` also provides a `markup` method that returns a <a href="Documenter.html">`Documenter`</a>, which allows you to send
 ...
 * create a matching tag for `WordSpec`s like this:
 * 
 *
 * {{{  <!-- class="stHighlight" -->
 ...
 * create a `Tag` object. A tag annotation (on the JVM, not Scala.js) allows you to tag all the tests of a `WordSpec` in
 ...
 * lifecycle methods defined in trait <a href="Suite.html#lifecyle-methods">`Suite`</a>.
 * 
 *
 ...
 * of `withFixture`, and let it invoke the test function rather than invoking the test function directly. Instead of writing
 ...
 * Each test in a `fixture.WordSpec` takes a fixture as a parameter, allowing you to pass the fixture into
 ...
 * the fixture object to the `toNoArgTest` method of `OneArgTest`. Instead of
 ...
 * the objects.  For more information on the `withFixture(OneArgTest)` technique, see the <a href="fixture/WordSpec.html">documentation for `fixture.WordSpec`</a>.
 * 
 *
 ...
 * between different `WordSpec`s, however, you could also define them in a separate trait that is mixed into each `WordSpec`
 * that uses them.
 * 
 *
