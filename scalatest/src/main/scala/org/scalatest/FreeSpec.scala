 ...
 * '''Recommended Usage''':
 * Because it gives absolute freedom (and no guidance) on how specification text should be written, `FreeSpec` is a good choice for teams experienced 
 ...
 *
 * One of the parameters to `FreeSpec`'s `run` method is a <a href="Reporter.html">`Reporter`</a>, which
 ...
 * the <a href="GivenWhenThen.html">`GivenWhenThen`</a> trait provides methods that use the implicit `info` provided by `FreeSpec`
 ...
 *
 * `FreeSpec` also provides a `markup` method that returns a <a href="Documenter.html">`Documenter`</a>, which allows you to send
 ...
 * create a matching tag for `FreeSpec`s like this:
 * 
 *
 * {{{  <!-- class="stHighlight" -->
 ...
 * an `apply` method that executes the test, also includes the test name and the <a href="ConfigMap">config
 ...
 * the objects.  For more information on the `withFixture(OneArgTest)` technique, see the <a href="fixture/FreeSpec.html">documentation for `fixture.FreeSpec`</a>.
 * 
 *
 ...
 * between different `FreeSpec`s, however, you could also define them in a separate trait that is mixed into each `FreeSpec`
 * that uses them.
 * 
 *
