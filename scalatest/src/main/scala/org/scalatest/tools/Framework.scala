 ...
 *
 * <ul>
 *   <li>Specified behavior of single instance of `Runner` per project run (non-fork), and a new `done` method</li>
 *   <li>API to return nested tasks</li>
 *   <li>API to support test execution in `fork` mode</li>
 *   <li>Selector API to selectively run tests</li>
 *   <li>Added new `Ignored`, `Canceled` and `Pending` status</li>
 ...
 * and this gives ScalaTest a good place to perform setup tasks, such as initializing `Reporter`s.
 * 
 *
 * There's also a new `done` on `Runner` interface, which in turns provide a good spot for ScalaTest to perform
 * cleanup tasks, such as disposing the `Reporter`s.  [[org.scalatest.tools.HtmlReporter `HtmlReporter`]] depends
 ...
 * 
 *
 * ===API to return nested `Suite`s as sbt `Task`s===
 *
 * In new Framework API, a new concept of <a href="https://github.com/sbt/test-interface/blob/master/src/main/java/sbt/testing/Task.java">`Task`</a>
 * was introduced. A `Task` has an `execute` method that can return more `Task`s for execution.  ScalaTest does not utilize this
 ...
 * sbt 0.13.  With API provided with new Framework API, ScalaTest creates real `Reporter`s in the main process, and uses `SocketReporter`
 * in forked process to send events back to the main process, and get processed by real `Reporter`s at the main process.  All of this is transparent
 ...
 *
 * <ul>
 *   <li>[[org.scalatest.tags.CPU `CPU`]]</li>
 *   <li>[[org.scalatest.tags.Disk `Disk`]]</li>
 *   <li>[[org.scalatest.tags.Network `Network`]]</li>
 * </ul>
 *
...
class Framework extends SbtFramework {
  ...
  /**
   * An array of `Fingerprint`</a>s that specify how to identify ScalaTest's test classes during
   * discovery.
   *
