...
/**
 * Implementation trait for class `FeatureSpec`, which represents
 * a suite of tests in which each test represents one ''scenario'' of a
 * ''feature''.
 * 
 * <a href="FeatureSpec.html">`FeatureSpec`</a> is a class, not a
 * trait, to minimize compile time given there is a slight compiler overhead to
 * mixing in traits compared to extending classes. If you need to mix the
 * behavior of `FeatureSpec` into some other class, you can use this
 * trait instead, because class `FeatureSpec` does nothing more than
