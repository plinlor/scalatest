...
/**
 * Implementation trait for class `AsyncWordSpec`, which facilitates a &ldquo;behavior-driven&rdquo; style of development (BDD), in which tests
 * are combined with text that specifies the behavior the tests verify.
 *
 * <a href="AsyncWordSpec.html">`AsyncWordSpec`</a> is a class, not a trait, to minimize compile time given there is a slight compiler overhead to
 * mixing in traits compared to extending classes. If you need to mix the behavior of `AsyncWordSpec`
 * into some other class, you can use this trait instead, because class `AsyncWordSpec` does nothing more than extend this trait and add a nice `toString` implementation.
 * 
 *
 * See the documentation of the class for a <a href="AsyncWordSpec.html">detailed overview of `AsyncWordSpec`</a>.
 * 
 *
 * @author Bill Venners
 */
