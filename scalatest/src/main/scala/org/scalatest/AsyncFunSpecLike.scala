...
/**
 * Implementation trait for class `AsyncFunSpec`, which
 ...
 * verify.
 *
 * <a href="AsyncFunSpec.html">`AsyncFunSpec`</a> is a class, not a trait,
 * to minimize compile time given there is a slight compiler overhead to
 * mixing in traits compared to extending classes. If you need to mix the
 * behavior of `AsyncFunSpec` into some other class, you can use this
 * trait instead, because class `AsyncFunSpec` does nothing more than
...
trait AsyncFunSpecLike extends AsyncTestSuite with AsyncTestRegistration with Informing with Notifying with Alerting with Documenting { thisSuite =>
   ...
   * the `Set` of tags for the test. If this `FunSpec` contains no tags, this method returns an empty `Map`.
   *
