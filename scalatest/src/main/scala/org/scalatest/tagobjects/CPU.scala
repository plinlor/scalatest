 ...
 *
 * The corresponding tag annotation for this tag object is `org.scalatest.tags.CPU`.
 * This tag object can be used to tag test functions (in style traits other than `Spec`, in which tests are methods not functions) as being CPU-intensive.
