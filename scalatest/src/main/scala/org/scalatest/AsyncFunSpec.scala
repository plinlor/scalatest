 ...
 * using a style consistent with traditional `FunSpec` tests.
 *
 ...
 * }}}
 *
 * An `AsyncFunSpec` contains ''describe clauses'' and tests. You define a describe clause
 * with `describe`, and a test with either `it` or `they`. 
 * `describe`,  `it`, and `they` are methods, defined in
 * `AsyncFunSpec`, which will be invoked
 ...
 * and testing. In the previous example, `"addSoon"` and `"addNow"` are
 ...
 * }}}
 *
 * An `AsyncFunSpec`'s lifecycle has two phases: the ''registration'' phase and the
 ...
 * 
 *
 * Tests can only be registered with the `it` method while the `AsyncFunSpec` is
 * in its registration phase. Any attempt to register a test after the `AsyncFunSpec` has
 * entered its ready phase, ''i.e.'', after `run` has been invoked on the `AsyncFunSpec`,
 * will be met with a thrown `TestRegistrationClosedException`. The recommended style
 * of using `AsyncFunSpec` is to register tests during object construction as is done in all
 ...
 *
 * `AsyncFunSpec` extends <a href="AsyncTestSuite.html">`AsyncTestSuite`</a>, which provides an
 * implicit `scala.concurrent.ExecutionContext`
 * named `executionContext`. This
 * execution context is used by `AsyncFunSpec` to 
 ...
 * traditional <a href="FunSpec.html">`FunSpec`</a> with
 ...
 *
 * By default (unless you mix in `ParallelTestExecution`), tests in an `AsyncFunSpec` will be executed one after
 ...
 * 
 *
 * If you want the tests of an `AsyncFunSpec` to be executed in parallel, you
 ...
 * run in parallel very much like traditional (such as <a href="FunSpec.html">`FunSpec`</a>) tests run in
 ...
 * good intention of resurrecting the test at a later time, `AsyncFunSpec` provides registration
 * methods that start with `ignore` instead of `it` or `they`. For example, to temporarily
 * disable the test with the text `"will eventually compute a sum of passed Ints"`, just
 ...
 *
 * One of the parameters to `AsyncFunSpec`'s `run` method is a `Reporter`, which
 * will collect and report information about the running suite of tests.
 * Information about suites and tests that were run, whether tests succeeded or failed, 
 * and tests that were ignored will be passed to the `Reporter` as the suite runs.
 * Most often the reporting done by default by `AsyncFunSpec`'s methods will be sufficient, but
 ...
 *
 * `AsyncFunSpec` also provides a `markup` method that returns a <a href="Documenter.html">`Documenter`</a>, which allows you to send
 ...
 * 
 *
 * You can define a behavior function that encapsulates these shared tests inside the `AsyncFunSpec` that uses them. If they are shared
 ...
 * }}}
 *
 * Given these behavior functions, you could invoke them directly, but `AsyncFunSpec` offers a DSL for the purpose,
