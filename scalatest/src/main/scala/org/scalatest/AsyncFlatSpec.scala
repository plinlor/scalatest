 ...
 * using a style consistent with traditional `FlatSpec` tests.
 *
 * <table><tr><td class="usage">
 * '''Recommended Usage''':
 * `AsyncFlatSpec` is intended to enable users of <a href="FlatSpec.html">`FlatSpec`</a>
 ...
 * 
 *
 * Trait `AsyncFlatSpec` is so named because
 ...
 * }}}
 *
 * The initial test in this example demonstrates the use of an explicit `behavior of` clause, which establishes
 * `addSoon` as the subject.  The second test demonstrates the alternate syntax of replacing the first `it`
 * with the subject string, in this case, `"addNow"`.
 * As with traditional `FlatSpec`s, you can use `must` or `can` as well as `should`.
 * For example, instead of `it should "eventually`..., you could write
 * `it must "eventually`... or `it can "eventually`....
 * You can also write `they` instead of `it`. See the documentation for <a href="FlatSpec.html">`FlatSpec`</a> for
 * more detail.
 * 
 *
 ...
 * }}}
 *
 * An `AsyncFlatSpec`'s lifecycle has two phases: the ''registration'' phase and the
 ...
 * 
 *
 * Tests can only be registered with the `it` method while the `AsyncFlatSpec` is
 * in its registration phase. Any attempt to register a test after the `AsyncFlatSpec` has
 * entered its ready phase, ''i.e.'', after `run` has been invoked on the `AsyncFlatSpec`,
 * will be met with a thrown `TestRegistrationClosedException`. The recommended style
 * of using `AsyncFlatSpec` is to register tests during object construction as is done in all
 ...
 *
 * `AsyncFlatSpec` extends <a href="AsyncTestSuite.html">`AsyncTestSuite`</a>, which provides an
 * implicit `scala.concurrent.ExecutionContext`
 * named `executionContext`. This
 * execution context is used by `AsyncFlatSpec` to 
 ...
 * traditional <a href="FlatSpec.html">`FlatSpec`</a> with
 ...
 *
 * By default (unless you mix in `ParallelTestExecution`), tests in an `AsyncFlatSpec` will be executed one after
 ...
 * 
 *
 * If you want the tests of an `AsyncFlatSpec` to be executed in parallel, you
 ...
 * run in parallel very much like traditional (such as <a href="FlatSpec.html">`FlatSpec`</a>) tests run in
 ...
 * good intention of resurrecting the test at a later time, `AsyncFlatSpec` provides two ways
 ...
 * }}}
 *
 * In the first test, `ignore` is used instead of `it`.
 * In the second test, which uses the shorthand notation, no `it` exists to change into `ignore`.
 * To ignore such tests, you must instead change `in` to `ignore`, as shown in the above example.
 ...
 *
 * One of the parameters to `AsyncFlatSpec`'s `run` method is a <a href="Reporter.html">`Reporter`</a>, which
 * will collect and report information about the running suite of tests.
 * Information about suites and tests that were run, whether tests succeeded or failed, 
 * and tests that were ignored will be passed to the `Reporter` as the suite runs.
 * Most often the reporting done by default by `AsyncFlatSpec`'s methods will be sufficient, but
 ...
 * the <a href="GivenWhenThen.html">`GivenWhenThen`</a> trait provides methods that use the implicit `info` provided by `AsyncFlatSpec`
 ...
 *
 * `AsyncFlatSpec` also provides a `markup` method that returns a <a href="Documenter.html">`Documenter`</a>, which allows you to send
 ...
 *
 * An `AsyncFlatSpec`'s tests may be classified into groups by ''tagging'' them with string names.
 * As with any suite, when executing an `AsyncFlatSpec`, groups of tests can
 * optionally be included and/or excluded. To tag an `AsyncFlatSpec`'s tests,
 ...
 * create a matching tag for `AsyncFlatSpec`s like this:
 * 
 *
 * {{{  <!-- class="stHighlight" -->
 ...
 * create a `Tag` object. A tag annotation (on the JVM, not Scala.js) allows you to tag all the tests of an `AsyncFlatSpec` in
 ...
 * method defined in trait <a href="AsyncTestSuite.html">`AsyncTestSuite`</a>, a supertrait of `AsyncFlatSpec`.
 * 
 *
 * Trait `AsyncFlatSpec`'s `runTest` method passes a no-arg async test function to
 ...
 * the <a href="fixture/AsyncFlatSpec.html">documentation for `fixture.AsyncFlatSpec`</a>.
 * 
 *
 ...
 * To accomplish this in an `AsyncFlatSpec`, you first place shared tests in
 * ''behavior functions''. These behavior functions will be
 * invoked during the construction phase of any `AsyncFlatSpec` that uses them, so that the tests they contain will
 ...
 * stack fixture to use when running the tests. So in your `AsyncFlatSpec` for `StackActor`, you'd invoke the
 ...
 * 
 *
 * You can define a behavior function that encapsulates these shared tests inside the `AsyncFlatSpec` that uses them. If they are shared
 * between different `AsyncFlatSpec`s, however, you could also define them in a separate trait that is mixed into
 * each `AsyncFlatSpec` that uses them.
 ...
 * }}}
 *
 * Given these behavior functions, you could invoke them directly, but `AsyncFlatSpec` offers a DSL for the purpose,
 ...
 * Although in an `AsyncFlatSpec`, the `behavior of` clause is a nesting construct analogous to
 ...
 * `AsyncFlatSpec`, you'll need to pass in a prefix or suffix string to add to each test name. You can call
 ...
 *
 * <ul>
 * <li>`A Stack actor (when non-empty) should return non-empty StackInfo when Size is fired at non-empty stack actor: almost empty stack actor`</li>
 * <li>`A Stack actor (when non-empty) should return before and after StackInfo that has existing size and lastItemAdded as top when Peek is fired at non-empty stack actor: almost empty stack actor`</li>
 * <li>`A Stack actor (when non-empty) should return before and after StackInfo that has existing size - 1 and lastItemAdded as top when Pop is fired at non-empty stack actor: almost empty stack actor`</li>
 * </ul>
 *
 ...
 *
 * <ul>
 * <li>`A Stack actor (when non-empty) should return non-empty StackInfo when Size is fired at non-empty stack actor: almost full stack actor`</li>
 * <li>`A Stack actor (when non-empty) should return before and after StackInfo that has existing size and lastItemAdded as top when Peek is fired at non-empty stack actor: almost full stack actor`</li>
 * <li>`A Stack actor (when non-empty) should return before and after StackInfo that has existing size - 1 and lastItemAdded as top when Pop is fired at non-empty stack actor: almost full stack actor`</li>
 * </ul>
 */
