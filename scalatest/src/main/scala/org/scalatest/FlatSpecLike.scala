...
/**
 * Implementation trait for class `FlatSpec`, which facilitates a
 * &ldquo;behavior-driven&rdquo; style of development (BDD), in which tests
 * are combined with text that specifies the behavior the tests verify.
 * 
 * <a href="FlatSpec.html">`FlatSpec`</a> is a class, not a trait,
 * to minimize compile time given there is a slight compiler overhead to
 * mixing in traits compared to extending classes. If you need to mix the
 * behavior of `FlatSpec` into some other class, you can use this
 * trait instead, because class `FlatSpec` does nothing more than
