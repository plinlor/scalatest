 ...
 * '''Recommended Usage''':
 * `AsyncFunSuite` is intended to enable users of <a href="FunSuite.html">`FunSuite`</a>
 ...
 * }}}
 *
 * &ldquo;`test`&rdquo; is a method, defined in `AsyncFunSuite`, which will be invoked
 * by the primary constructor of `AddSuite`. You specify the name of the test as
 * a string between the parentheses, and the test code itself between curly braces.
 * The test code is a function passed as a by-name parameter to `test`, which registers
 * it for later execution. The result type of the by-name in an `AsyncFunSuite` must
 * be `Future[Assertion]`. 
 * 
 *
 ...
 * }}}
 * 
 * When `AddSuite` is constructed, the second test will be implicitly converted to
 ...
 * }}}
 *
 * An `AsyncFunSuite`'s lifecycle has two phases: the ''registration'' phase and the
 ...
 * 
 *
 * Tests can only be registered with the `test` method while the `AsyncFunSuite` is
 * in its registration phase. Any attempt to register a test after the `AsyncFunSuite` has
 * entered its ready phase, ''i.e.'', after `run` has been invoked on the `AsyncFunSuite`,
 * will be met with a thrown `TestRegistrationClosedException`. The recommended style
 * of using `AsyncFunSuite` is to register tests during object construction as is done in all
 ...
 *
 * `AsyncFunSuite` extends <a href="AsyncTestSuite.html">`AsyncTestSuite`</a>, which provides an
 * implicit `scala.concurrent.ExecutionContext`
 * named `executionContext`. This
 * execution context is used by `AsyncFunSuite` to 
 ...
 * traditional <a href="FunSuite.html">`FunSuite`</a> with
 ...
 *
 * By default (unless you mix in `ParallelTestExecution`), tests in an `AsyncFunSuite` will be executed one after
 ...
 * 
 *
 * If you want the tests of an `AsyncFunSuite` to be executed in parallel, you
 ...
 * run in parallel very much like traditional (such as <a href="FunSuite.html">`FunSuite`</a>) tests run in
 ...
 * good intention of resurrecting the test at a later time, `AsyncFunSuite` provides registration
 ...
 * Thus, marking the `AddSuite` in the above example with the `@Ignore` tag annotation means that both tests
 * in the class will be ignored. If you run the above `AddSuite` in the Scala interpreter, you'll see:
 * 
 *
 * {{{  <!-- class="stREPL" -->
 * scala&gt; org.scalatest.run(new AddSuite)
 ...
 * to change `test` to `ignore` at each test site. To make a suite non-discoverable on Scala.js, ensure it
 ...
 *
 * One of the parameters to `AsyncFunSuite`'s `run` method is a `Reporter`, which
 * will collect and report information about the running suite of tests.
 * Information about suites and tests that were run, whether tests succeeded or failed, 
 * and tests that were ignored will be passed to the `Reporter` as the suite runs.
 * Most often the reporting done by default by `AsyncFunSuite`'s methods will be sufficient, but
 ...
 *
 * `AsyncFunSuite` also provides a `markup` method that returns a <a href="Documenter.html">`Documenter`</a>, which allows you to send
 ...
 *
 * An `AsyncFunSuite`'s tests may be classified into groups by ''tagging'' them with string names.
 * As with any suite, when executing an `AsyncFunSuite`, groups of tests can
 * optionally be included and/or excluded. To tag an `AsyncFunSuite`'s tests,
 ...
 * create a matching tag for `AsyncFunSuite`s like this:
 * 
 *
 * {{{  <!-- class="stHighlight" -->
 ...
 * create a `Tag` object. A tag annotation (on the JVM, not Scala.js) allows you to tag all the tests of an `AsyncFunSuite` in
 ...
 * method defined in trait <a href="AsyncTestSuite.html">`AsyncTestSuite`</a>, a supertrait of `AsyncFunSuite`.
 * 
 *
 * Trait `AsyncFunSuite`'s `runTest` method passes a no-arg async test function to
 ...
 * the <a href="fixture/AsyncFunSuite.html">documentation for `fixture.AsyncFunSuite`</a>.
 * 
 *
 ...
 * To accomplish this in an `AsyncFunSuite`, you first place shared tests in
 * ''behavior functions''. These behavior functions will be
 * invoked during the construction phase of any `AsyncFunSuite` that uses them, so that the tests they contain will
 ...
 * stack fixture to use when running the tests. So in your `AsyncFunSuite` for `StackActor`, you'd invoke the
 ...
 * 
 *
 * You can define a behavior function that encapsulates these shared tests inside the `AsyncFunSuite` that uses them. If they are shared
 * between different `AsyncFunSuite`s, however, you could also define them in a separate trait that is mixed into
 * each `AsyncFunSuite` that uses them.
 ...
 * }}}
 *
 * Given these behavior functions, you could invoke them directly, but `AsyncFunSuite` offers a DSL for the purpose,
 ...
 * In a `AsyncFunSuite` there is no nesting construct analogous to
 * `AsyncFunSpec`'s `describe` clause.
 ...
 * `AsyncFunSuite`, you'll need to pass in a prefix or suffix string to add to each test name. You can call
